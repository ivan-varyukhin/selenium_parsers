...
***sum_drop-down_list.py***
    ...
    Затем вставит получившийся результат в поле на сайте, нажать кнопку и получить результат.
...
***open_window_with_correct_size.py***
    ...
    Есть 2 списка с размерами  size_x и size_y, при единственном правильном сочетании размеров из этих списков, появится число;
    Результат появится в id="result".
...
***open_window_and_get_correct_size.py***
    ...
    где размеры указаны без учёта размеров рамок браузера, т.е. необходимо указать размер рабочей области браузера.
...
***lick_buttons_and_sum_titles.py***
    ...
    На сайте есть 10 buttons, каждый button откроет сайт в новой вкладке, каждая вкладка имеет в title уникальное число.
...
***open_tabs_click_and_sum.py***
    ...
    Необходимо открыть с помощью Selenium все сайты во вкладках (.window_handles), пройти в цикле по каждой вкладке, нажать на chekbox и получить код.
...
***catch_title_and_get_result.py***
    ...
    На сайте есть кнопка, которая становится активной после загрузки страницы со случайной задержкой (от 1 до 3 сек).
